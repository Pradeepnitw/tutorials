...
public class DungeonGame {
	...
	public int calculateMinimumHP(int[][] dungeon) {
		...
		// Boundary condition for Bottom right
	...
	/*********************
	 * Solution with Upper bound of knight's health, and calcuate that upper bound
	 * 
	...
	/*
	 * Used in calculateMinimumHPHeapWithUpperBound
	 */
	private class DungeonGamePath implements Comparable<DungeonGamePath> {
		public int xIdx;
		public int yIdx;
		public int currHp;
		public int maxHp;
		
		public int compareTo(DungeonGamePath o) {
			// Smallest element to the top of priority queue
			// If the value is the same, than return the one closest to the end
			int res = this.maxHp - o.maxHp;
			int distance = this.xIdx - o.xIdx + this.yIdx - o.yIdx;
			
			return res/5 + distance;
		}
	}

	/*
	 * If the knight's heath have upper bound, then for each path we need to calculate the
	 * maximum subarray sum.
	 */
	public int calculateMinimumHPHeapWithUpperBound(int[][] dungeon) {
		PriorityQueue<DungeonGamePath> pq = new PriorityQueue<DungeonGamePath>();
		
		DungeonGamePath[][] dgPath = new DungeonGamePath[dungeon.length][dungeon[0].length];
		for (int i = 0; i < dgPath.length; i++) {
			for (int j = 0; j < dgPath[0].length; j++) {
				dgPath[i][j] = new DungeonGamePath();
				dgPath[i][j].xIdx = i;
				dgPath[i][j].yIdx = j;
			}
		}
		
		DungeonGamePath start = dgPath[0][0];
		start.currHp -= dungeon[0][0];
		start.maxHp = start.currHp;
		pq.add(start);
		
		DungeonGamePath last = dgPath[dungeon.length-1][dungeon[0].length-1];
		last.maxHp = Integer.MAX_VALUE;
		if (this.verbose) {
			System.out.println("last[" + last.xIdx + "][" + last.yIdx + "] dungeon.length=" + dungeon.length + " dungeon[0].length=" + dungeon[0].length);
		}
		
		while (pq.peek().maxHp < last.maxHp) {
			if (this.verbose) {
				System.out.println("Size of pq=" + pq.size() + " Max hp=" + pq.peek().maxHp + " last' max hp=" + dgPath[dungeon.length-1][dungeon[0].length-1].maxHp);
			}
			DungeonGamePath top = pq.poll();
			
			if (this.verbose) {
				System.out.println("Top[" + top.xIdx + "][" + top.yIdx + "] maxHP=" + top.maxHp);
			}
			
			// If top reaches the end, update the last.maxHp, but not necessarily end this while loop
			if (top.xIdx == last.xIdx && top.yIdx == last.yIdx) {
				if (this.verbose) {
					System.out.println("Reaching last step");
				}
				last.maxHp = top.maxHp;
				if (pq.size() != 0)
					continue;
				else break;
			}
			...
			// Go down one step
			if (top.xIdx+1 <= last.xIdx) {
				DungeonGamePath down = new DungeonGamePath();
				down.xIdx = top.xIdx + 1;
				down.yIdx = top.yIdx;
				down.currHp = top.currHp - dungeon[down.xIdx][down.yIdx];
				
				if (top.maxHp < down.currHp) {
					down.maxHp = down.currHp;
				} else
					down.maxHp = top.maxHp;
				
				if (down.currHp < 0)
					down.currHp = 0;
				
				if (this.verbose) {
					System.out.println("Adding...Down's x=" + down.xIdx + " y=" + down.yIdx + " maxHp=" + down.maxHp);
				}
			...
			// Go right one step
			if (top.yIdx+1 <= last.yIdx) {
				DungeonGamePath right = new DungeonGamePath();
				right.xIdx = top.xIdx;
				right.yIdx = top.yIdx + 1;
				right.currHp = top.currHp - dungeon[right.xIdx][right.yIdx];
				
				if (top.maxHp < right.currHp) {
					right.maxHp = right.currHp;
				} else
					right.maxHp = top.maxHp;
				
				if (right.currHp < 0)
					right.currHp = 0;
				
				if (this.verbose) {
					System.out.println("Adding...Right's x=" + right.xIdx + " y=" + right.yIdx + " maxHp=" + right.maxHp);
				}
	 ...
	 * Solution with Upper bound of knight's health, and calcuate that upper bound
	 * 
	 * Solution 2: calculate all possible path recursively
	 *
	 *********************/
	
	public int calculateMinmumHPRecursiveWithUppderBound(int[][] dungeon) {
		if (dungeon == null)
			return -1;
		
		ArrayList<int[]> result = calculateMinmumHPRecursive(dungeon, 0, 0);
	...
	public ArrayList<int[]> calculateMinmumHPRecursive(int[][] dg, int x, int y) {
		ArrayList<int[]> res = new ArrayList<int[]>();
		
		// Last step reached
		if (x == dg.length-1 && y == dg[0].length-1) {
			int[] hp = new int[2];
			hp[CURRENT] = -1 * dg[x][y];
			hp[MAX] = -1 * dg[x][y];
			res.add(hp);
		...
		// Go down one step
		if (x + 1 < dg.length) {
			res.addAll(calculateMinmumHPRecursive(dg, x+1, y));
		}
		
		// Go right one step
		if (y + 1 < dg[0].length) {
			res.addAll(calculateMinmumHPRecursive(dg, x, y+1));
		}
		...
		if (this.verbose) {
			System.out.println("x=" + x + " y=" + y + " numOfRes=" + res.size());
		}
	 ...
	 * Solution with Upper bound of knight's health, and calcuate that upper bound
	 * 
	 * Solution 2: calculate all possible path iteratively
	 *
	 *********************/
	 ...
	 * on 9 12
	 */
	public int calculateMinmumHPBruteForceWithUpperBound(int[][] dungeon) {
		if (dungeon == null)
			return -1;
		
		LinkedList<LinkedList<int[]>> allPath = scanAllPossiblePathIterative(dungeon);

		int minMaxHp = Integer.MAX_VALUE;
		for (LinkedList<int[]> path : allPath) {
			int currentHp = 0;
			int maxHp = 0;
			for (int[] idx : path) {
				currentHp -= dungeon[idx[0]][idx[1]];
				if (currentHp > maxHp)
					maxHp = currentHp;
				if (currentHp < 0)
					currentHp = 0;
			...
			if (maxHp < minMaxHp)
				minMaxHp = maxHp;
			System.out.println("Path Max HP=" + maxHp);
		}
			
		return minMaxHp;
	}

	public LinkedList<LinkedList<int[]>> scanAllPossiblePathIterative(int[][] map) {
		LinkedList<LinkedList<int[]>> queue = new LinkedList<LinkedList<int[]>>();
		...
		while(!queue.isEmpty()) {
			...
			if(queue.peek().getLast()[0] == map.length - 1 && queue.peek().getLast()[1] == map[0].length - 1)
                                break;
			path = queue.pop();
			int[] idx = path.getLast();
		
			System.out.println("on " + idx[0] + " " + idx[1]);	
				
			if(map.length - 1 > idx[0]) {
				LinkedList<int[]> newPath = new LinkedList<int[]>();
				newPath.addAll(path);
				newPath.add(new int[]{idx[0]+1, idx[1]});
				//System.out.println("Add:" + new Integer(idx[0]+1) + " " + idx[1]);
				queue.add(newPath);
			}

			if(map[0].length - 1 > idx[1]) {
				LinkedList<int[]> newPath = new LinkedList<int[]>();
				newPath.addAll(path);
				newPath.add(new int[]{idx[0], idx[1]+1});
				//System.out.println("Add:" + idx[0] + " " + new Integer(idx[1]+1));
				queue.add(newPath);
			}
		}
...
//		System.out.println(obj.calculateMinmumHPBruteForceWithUpperBound(dg));
//		System.out.println(obj.calculateMinmumHPRecursiveWithUppderBound(dg));
		
		
//		int[][] test = new int[][]{{0,0}};
//		System.out.println("test dimenstion: " + test.length + " " + test[0].length);
//		System.out.println(obj.calculateMinimumHPHeapWithUpperBound(test));
//		System.out.println(obj.calculateMinimumHPHeapWithUpperBound(dg));
		
		System.out.println(obj.calculateMinimumHP(ar));
	}
}
